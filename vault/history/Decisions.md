...
**Why (2026-06-11, founder layout):** The designer shipped an 8-card set (`utils/palm-react/cards-export/`, framer-motion async/await loops). The founder split them by theme: **Explore** gets the 4 "producto" cards (gastos, cc, goals, portfolio) as a single aligned 4-up row; **Comparativa ("¿Por qué Palm?")** gets the 4 "confianza" cards (mass, privacy, drain, zero) in an identical 4-up row (same `pv-explore__grid` + 1400px container as Explore; founder asked both sections to share the structure) — replacing the old bento tiles (hero +$4,2M chart, donut 73%, split de carteras, legacy `.pcard--zero`), whose messages the new cards retell. The export's gradient border on the card shell was removed (flat bg) and `--pad-y` went 96→64px (48px mobile) with the per-section padding special-cases (.problem bottom, .explore top) deleted — spacing between sections is uniform now. The old `.pcard`/`.cc-`/`.g2/3/4-`/`gastos-` CSS was deleted from `sections.css` once nothing used it; the `pv-` prefix on the new classes is historical (it kept them from colliding while both generations coexisted) — keep it.
**Update (2026-06-11, founder):** tras tres rondas de iteración en `/cards-propuestas`, **5 de las 8 cards fueron reemplazadas por las animaciones propuestas**, tomando el lugar de los archivos de producción (mismo nombre de archivo, clases `pv-p*-`): en Explore solo **CardCC** ("el resaltador" — doc crema, banda dorada, lupa al costado); en Comparativa **las cuatro** — CardMass (scroll de clientes que frena y selecciona "vos."), CardPrivacy (cubos rojos + cablecitos + escudo sobre el recuadro), CardDrain (tanque "TU PLATA", headline nuevo "Tu plata, sin goteras." porque el original era casi igual al de Zero), CardZero (ticket que se escribe línea por línea y se sella 0%). Gastos/Goals/Portfolio siguen siendo las del export del diseñador; sus propuestas no promovidas viven en `components/palm-cards/proposals/` (+ página `/cards-propuestas`).
